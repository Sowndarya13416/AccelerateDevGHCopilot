...
// Add the correct namespace for JsonData below (update if needed)
using Library.Infrastructure.Data; // or the actual namespace where JsonData is defined
...
public class ConsoleApp
{
    ...
    )
    {
        ...
        _jsonData = jsonData; // Assign to field
    }
...
async Task<ConsoleState> SearchBooks()
{
    ...
    // Search for the book in Books.json (case-insensitive)
    ...
    // Find the corresponding BookItem
    var bookItem = _jsonData.BookItems.FirstOrDefault(bi => bi.BookId == book.Id);

    if (bookItem == null)
    {
        Console.WriteLine($"No book item found for \"{bookTitle}\".");
        return ConsoleState.PatronDetails;
    }

    // Check for active loan (ReturnDate == null)
